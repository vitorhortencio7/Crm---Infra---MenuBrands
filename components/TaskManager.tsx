...
export const TaskManager: React.FC<TaskManagerProps> = ({
    ...
    const handleSaveTask = () => {
        ...
        if (editingTaskId) {
            // If editing, preserve the status from original task
    ...
    const myTasks = useMemo(() => {
        ...
        if (filterStatus === 'pending') return userTasks.filter(t => t.status !== 'done');
        if (filterStatus === 'completed') return userTasks.filter(t => t.status === 'done');
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-20">
                    {myTasks.length === 0 ? (
                        ...
                        <div className="space-y-3">
                            {myTasks.map(task => {
                                const isLate = task.status !== 'done' && isOverdue(task.dueDate);
                                return (
                                    <div
                                        key={task.id}
                                        className={`
                                        group bg-white dark:bg-slate-800 p-4 rounded-xl border transition-all duration-200 relative overflow-hidden
                                        ${task.status === 'done' ? 'opacity-60 border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-800' : 'border-gray-200 dark:border-slate-700 hover:border-red-200 dark:hover:border-red-900/50 hover:shadow-md'}
                                    `}
                                    >
                                        {/* Left Border Status Indicator */}
                                        <div className={`absolute left-0 top-0 bottom-0 w-1 ${task.status === 'done' ? 'bg-emerald-400' : isLate ? 'bg-red-500' : 'bg-gray-200 dark:bg-slate-600'}`}></div>
